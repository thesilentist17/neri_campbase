  ...
  return (
    <main className="min-h-screen bg-gray-50 font-sans flex flex-col relative">
      ...
      <div className="flex-grow">
        ...
        {/* 🟢 ОНОВЛЕНА ШАПКА З АКУРАТНИМИ КНОПКАМИ */}
        <div className="bg-white border-b border-gray-200 p-4 sm:p-6 sticky top-0 z-50 shadow-sm">
          <div className="max-w-4xl mx-auto flex justify-between items-center gap-2">
            
            <button onClick={() => window.history.back()} className="text-gray-500 hover:text-[#FDB8D3] font-bold text-base sm:text-lg transition-colors shrink-0 flex items-center">
              ← Назад
            </button>
            
            {/* Блок з кнопками - без flex-wrap, суворо в один ряд */}
            <div className="flex gap-2 items-center justify-end">
              ...
              <button 
                ...
                title="Зберегти офлайн"
                className={`flex items-center justify-center gap-2 border font-bold w-11 h-11 sm:w-auto sm:h-auto sm:px-5 sm:py-2.5 rounded-xl transition-all shadow-sm shrink-0 ${
                  isDownloading 
              ...
              >
                {isDownloading ? (
                  <>
                    <svg className="animate-spin w-5 h-5 text-white shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <span className="hidden sm:inline text-sm">Зберігаємо...</span>
                  </>
                ) : (
                  <>
                    <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span className="hidden sm:inline text-sm">Офлайн</span>
                  </>
                )}
              </button>

              {/* Наша кнопка ShareButton */}
              <ShareButton 
